...
test.describe('MacroEngine', () => {
    ...
    test.describe('Comment macro', () => {
        test('should remove empty comments without a matching closing block', async ({ page }) => {
            const output = await evaluateWithEngine(page, 'A{{//}}B');
            expect(output).toBe('AB');
        });

        test('should discard multiple explicit comment arguments', async ({ page }) => {
            const output = await evaluateWithEngine(page, 'A{{//::a::b}}B');
            expect(output).toBe('AB');
        });

...
describe('getChatInfo', () => {
    ...
    test('reports unreadable chats without breaking the remaining scan', async () => {
        const eacces = Object.assign(new Error('EACCES: permission denied'), { code: 'EACCES' });
        jest.spyOn(fs.promises, 'stat').mockRejectedValue(eacces);
        const errorLog = jest.spyOn(console, 'error').mockImplementation(() => {});
        await expect(chats.getChatInfo(chatFile)).resolves.toEqual({});
        expect(errorLog).toHaveBeenCalledWith('Failed to read chat info:', chatFile, eacces);
    });
